...
void release_chair(int stud_id) {
  ...
  if(waiting_count > 0) {
    ...
    if(Students[i] == WAITING_FOR_TA) {
      ...
      acquire_TA(i); // give the TA to the waiting student.
    } else {
      // assert: there's people waiting but no student in the waiting state.
      assert(0);
    }
...
void *Student_thread_func(void *param)
{
  ...
  do {
    ...
    if(Students[i] == WAITING_FOR_TA) {
      ...
      printf("Student %d: %p: A student left the TAs office. He's given us the TA... waiting_count = %d.\n", i, tid, waiting_count);
    }
